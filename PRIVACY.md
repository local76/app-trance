...
rSaver only makes network requests in the following developer initiated contexts:
  Catalog Feeds: When launched, rSaver connects to the URLs defined in your feed_urls configuration (by default, the official GitHub repository registry.json) to fetch the list of available curated screensavers.
  Screensaver Downloads: When you explicitly request to download/install an online screensaver, rSaver connects directly to the screensaver's host URL to download the binary.
