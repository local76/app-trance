...
echo "=== Releasing trance v$VERSION ==="
...
if ! git diff --cached --quiet; then
    git commit -m "release: trance $VERSION"
else
...
git tag -a "v$VERSION" -m "trance v$VERSION"
...
if command -v gh &> /dev/null; then
    ...
    if [ ${#ASSETS[@]} -gt 0 ]; then
        gh_args=("release" "create" "v$VERSION" "${ASSETS[@]}" "--title" "trance v$VERSION" "--generate-notes")
...
echo "=== Release trance v$VERSION completed successfully! ==="
