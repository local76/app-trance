...
impl SystemMetrics {
    pub fn query() -> Self {
        ...
        SystemMetrics {
            ...
            dpi,
            window_dpi,
            dark_mode: crate::backend::sys_info::query_dark_mode(),
...
fn query_accent_color() -> Rgb {
    // Delegate to local sys_info (which uses DWM internally when "widgets" + "sys-info" features are enabled).
    // Convert ratatui Color::Rgb back to our local Rgb for the palette.
    match crate::backend::sys_info::get_dwm_accent_color() {
...
pub fn query_power_status() -> PowerStatus {
    // Delegate to local sys_info for the common power query logic.
    if let Some(p) = crate::backend::sys_info::query_power_status() {
        PowerStatus {
