...
You can install rIdle globally via the Windows Package Manager (WinGet):
winget install TourianDynamics.rIdle

What's New (Current Release)
This release aligns the visual styling, interfaces, and experience of rIdle and rMonitor under a unified dashboard design system:
  ...
  Console Tab Titles: Sets the active tab title to rIdle on startup and restores it upon exit.
...
What rIdle Does
rIdle manages the native Windows Screen Saver system by bridging standard OS-level registry settings with modern terminal-based configuration. Key capabilities include:
  Automatic Discovery: Scans Windows system folders (System32, SysWOW64, etc.) and a dedicated user folder in %APPDATA%\rIdle\screensavers for .scr executables.
  Config Sync Alerts: Automatically monitors registry changes in the background and hot-reloads rIdle if the system screensaver is modified externally (for example, via the native Windows Settings).
...
rIdle comes integrated with a catalog of retro terminal-style screensavers optimized for Windows 11 (now fully dynamic with live OS name + kernel logos pulled from the system):
...
rIdle acts as both a dashboard and a screensaver command-line handler.
rIdle.exe [OPTIONS] [COMMAND]
...
Commands:
  ...
  run : Launch the currently active screensaver in fullscreen mode (rIdle run).
...
Use the following shortcuts to navigate the dashboard:
  ...
  q / Esc : Quit rIdle or close active overlay popups.

Custom Preferences and Data Storage
All data is stored locally under your Windows user profile:
  rIdle Custom Preferences: Stored at %APPDATA%\rIdle\config.yaml (contains last-selected screensaver, prevent-sleep status, custom cycle interval, and catalog feed URLs).
  Screensaver Drop Path: Put custom .scr screensavers in %APPDATA%\rIdle\screensavers to have rIdle discover them.
  Logs File: Diagnostics are written to %APPDATA%\rIdle\rIdle.log so they do not clutter raw terminal outputs.

Custom Feeds:
To add custom online registry catalogs, open %APPDATA%\rIdle\config.yaml (or ~/.config/rIdle/config.yaml on Linux) and add your feed URLs separated by semicolons:
feed_urls: https://raw.githubusercontent.com/local76/rIdle/master/registry.json;https://example.com/custom-screensavers.json

Cross-platform support:
rIdle detects the current OS (`current_platform()`) and selects the right asset from the "downloads" map in the registry.
- Windows: .scr binary
- Linux: Picks the best variant from the downloads map:
  ...
  For the raw ELF, rIdle downloads it, installs the executable to `~/.xscreensaver/<name>`, chmod +x, and generates a minimal .xml descriptor under `~/.xscreensaver/config/<name>.xml` (option B — rIdle generates client-side so the published linux/ folder stays a simple collection of ELF + convenience packages).
- Packages (.deb / .rpm / .pkg.tar.zst): downloaded to the local rIdle cache (`~/.local/share/rIdle/screensavers/` or XDG equivalent). A `<name>.install.txt` sidecar is written with the exact command, and the TUI status bar shows the one-liner (e.g. `sudo pacman -U ...`). Yes, we include Arch (linux-arch + .pkg.tar.zst).
...
The optimized binary will be compiled to target/release/rIdle.exe. You can rename this to rIdle.scr to install it directly as a Windows screensaver!
